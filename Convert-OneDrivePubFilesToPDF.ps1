...
<#
.SYNOPSIS
    Converts Publisher files from the current user's OneDrive for Business folder.

.DESCRIPTION
    Locates the current user's locally synchronized OneDrive for Business folder,
    recursively finds .pub files, and passes them to Convert-PubFileToPdf. PDFs are
    created beside their source Publisher files.

    The OneDriveCommercial environment variable is preferred. If it is unavailable,
    the script checks the current user's OneDrive account registry settings for a
    Business account.

.PARAMETER TransactionLogPath
    Optional path to the CSV transaction log passed to Convert-PubFileToPdf.

.EXAMPLE
    .\Convert-OneDrivePubFilesToPDF.ps1 -Verbose

.EXAMPLE
    .\Convert-OneDrivePubFilesToPDF.ps1 -TransactionLogPath .\logs\onedrive.csv -Verbose
#>
...
if ([string]::IsNullOrWhiteSpace($oneDriveRoot)) {
    ...
    $businessAccounts = @(Get-ChildItem -Path $accountRoot -ErrorAction SilentlyContinue | Where-Object {
            $account = Get-ItemProperty -LiteralPath $_.PSPath -ErrorAction SilentlyContinue
            $account.PSObject.Properties.Name -contains 'UserFolder' -and
            -not [string]::IsNullOrWhiteSpace($account.UserFolder) -and
            $_.PSChildName -like 'Business*'
        })

    if ($businessAccounts.Count -gt 0) {
        $oneDriveRoot = (Get-ItemProperty -LiteralPath $businessAccounts[0].PSPath).UserFolder
    }
}

if ([string]::IsNullOrWhiteSpace($oneDriveRoot)) {
    throw 'Could not locate the current user''s OneDrive for Business folder. Ensure OneDrive is configured and synchronized.'
}

if (-not (Test-Path -LiteralPath $oneDriveRoot -PathType Container)) {
    throw "The OneDrive for Business folder does not exist or is unavailable: $oneDriveRoot"
}

$pubFiles = @(Get-ChildItem -LiteralPath $oneDriveRoot -Filter '*.pub' -File -Recurse)
Write-Verbose "OneDrive for Business folder: $oneDriveRoot"
Write-Verbose "Found $($pubFiles.Count) Publisher file(s)."

if ($pubFiles.Count -eq 0) {
    Write-Verbose 'No Publisher files found. Nothing to convert.'
    return
}
...
if ($PSBoundParameters.ContainsKey('TransactionLogPath')) {
    $conversionParams.TransactionLogPath = $TransactionLogPath
}

Convert-PubFileToPdf @conversionParams
